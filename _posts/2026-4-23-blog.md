...
The resulting assembly decoder is down under 50 bytes, which is small enough to fit in some tight places. The encoder/decoder together come in just under 200 bytes. A natural benefit of this data style is that the entropy counts come in rather low - generally around ~4.0! What's the catch? We're paying a hefty price in overhead. On average (since the skip value would be generated randomly), this increases the size of any payload by ~6x, which is costly.
